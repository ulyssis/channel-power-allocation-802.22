...
%
% handles = barweb([mean(powerHistory2,2)';mean(powerHistory,2)'], [1.96*std(powerHistory2,1,2)'/sqrt(n); 1.96*std(powerHistory,1,2)'/sqrt(n)], [], {'from LP'; 'from cvx'}, [], [], 'average transmission power over WBS', bone, 'y', {'Rand'; 'WhiteCat'; 'WhiteCase'; 'NoregretLearning'}, 2, 'plot')
	...
	if strcmp(legend_type, 'axis')
		for i = 1:numbars
			...
			for j = 1:length(xdata)
				text(xdata(j),  -0.03*ymax*1.1, bw_legend(i), 'Rotation', 30, 'fontsize', 12, 'HorizontalAlignment', 'right'); % 12 font, the second item is vertical distance between caption and x axis
			end
		end
