...
% Add second-order cone: p_i^k * q_i^k = 1
for r = 1:n*c
	Qc = zeros(3*n*c + 2*n^2*c, 3*n*c + 2*n^2*c);
	Qc(n*c+r, 2*n*c+2*n^2*c+r) = 1;
    
	model.quadcon(r).Qc = sparse(Qc);
	model.quadcon(r).q = zeros(3*n*c + 2*n^2*c, 1);
	model.quadcon(r).rhs = 1;
end
for r = 1:n*c
	Qc = zeros(3*n*c + 2*n^2*c, 3*n*c + 2*n^2*c);
	Qc(n*c+r, 2*n*c+2*n^2*c+r) = -1;
