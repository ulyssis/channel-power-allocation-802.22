...
% update channel usage based on part1
function [B, updateFlag] = selfishUpdate(su, B, P, Gtilde, m, GtildeAll, TVpower, delta, SUcellRadius, pathlossfactor, eta)
n = size(B, 1);
F = (B* B' ~= 0);
F = F - eye(size(B, 1));
    InterferenceonAll = Gtilde.* F * sum(B, 2)*eta + sum(GtildeAll(n+m+1:n+m+m, 1:n)'.* (B~=0) * TVpower, 2) + delta/n;
