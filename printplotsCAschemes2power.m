...
function printplotsCAschemes2power(plotLog, n, powerHistory, averageSinrHistory, SINR_ETs_random, SINR_ETs_whitecat, SINR_ETs_whitecase, SINR_ETs_lindo, SINR_ETs_noregret, SINR_ETs_PotentialGame_container)
...
figure (plotLog+3)
    ...
    hold on
    h6 = cdfplot(SINR_ETs_PotentialGame_container);
...
%% SINR average.
figure (plotLog+4)
averageSINR = [mean(SINR_ETs_random), mean(SINR_ETs_whitecat), mean(SINR_ETs_whitecase), mean(SINR_ETs_noregret), mean(SINR_ETs_lindo), mean(SINR_ETs_PotentialGame_container)];
stdSINR = [1.96*std(SINR_ETs_random,1)/sqrt(size(SINR_ETs_random, 2)), 1.96*std(SINR_ETs_whitecat,1)/sqrt(size(SINR_ETs_whitecat, 2)), 1.96*std(SINR_ETs_whitecase,1)/sqrt(size(SINR_ETs_whitecase, 2)), 1.96*std(SINR_ETs_noregret,1)/sqrt(size(SINR_ETs_noregret, 2)), 1.96*std(SINR_ETs_lindo,1)/sqrt(size(SINR_ETs_lindo, 2)), 1.96*std(SINR_ETs_PotentialGame_container,1)/sqrt(size(SINR_ETs_PotentialGame_container, 2))];
handle1 = barweb(averageSINR, stdSINR, [], [], [], [], 'Average transmission power of each WBS (W)', bone, 'y', {'Random Allocation'; 'whiteCat'; 'whiteCase'; 'Noregret Learning' ; 'optimization'; 'Potential Game'}, 2, 'plot');
set(handle1.legend,'Location','southeast', 'FontSize', 10, 'Color', 'R');
