...
for i = 1: c
    for j = 1: powerLevels
        ...
        Usu(j, i) = -part1 - part2 + part3; % minimize this!
    end
end

% find the workingChannel and working Power which result in the minimum Usu
 [level,channel,tem2]=find(Usu==min(Usu(:)));
 level=level(1);
 channel=channel(1);
% update the channel and power uage in matrix B. 
B(su,:) = zeros(1, c);
B(su, channel) = 4+(sum(P((su-1)*c + channel, channel))-4)*level/powerLevels;
