function [SINRofETs, averageSINR, error, worstSINR, fairness] = SINR_ETs_cellReSelection(B, n, GtildeETsSUs, nET, TVpower, delta)
    
    inf_ET_SU = repmat(GtildeETsSUs(1:n*nET, n*nET+1:n*nET+n), 1,w).*repmat((ones(n*nET,1)*sum(B, 2)'), 1,w); % n*nET x n, the power received from every SU
