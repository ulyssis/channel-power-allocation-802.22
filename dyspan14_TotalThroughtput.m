...
function [totalThroughput] = dyspan14_TotalThroughtput(chosenChannel, chosenClique, n, c, P_CVX, Gtilde, TVpower, delta, eta, SUcellRadius, pathlossfactor)
        ...
        % xxxxxx
        rcvdPowerMap = powerMap * SUcellRadius^(-pathlossfactor);
       
        InterferenceonAll = Gtilde.* F * sum(powerMap, 2)*eta + delta;
        part1 = log2(1+ sum(rcvdPowerMap, 2)./InterferenceonAll);
